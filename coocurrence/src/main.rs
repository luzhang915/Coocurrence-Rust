...
fn coocurrence_all(trace : &String, set:Vec<char>, window_bound:u32) {
...
fn main() {
	let data = fs::read_to_string("fixedwords.txt").expect("Unable to read file");
	let data = data.trim();
	coocurrence_all(&data.to_string(), vec!['A', 'B', 'C'], 10000);
}
